...
void FPLATEAUGmlSelectPanel::UpdateWindow(TWeakPtr<SWindow> MyWindow) {
    ...
    ScrollBox->AddSlot()[
        SNew(SVerticalBox)
            ...
            .ForegroundColor(FColor::White)
            .ButtonColorAndOpacity(FColor(10, 90, 80, 255))
            .OnClicked_Raw(this, &FPLATEAUGmlSelectPanel::OnBtnSampleXyzTilesClicked)
            .Content()
            [
                SNew(STextBlock)
                .Justification(ETextJustify::Center)
            .Margin(FMargin(0, 5, 0, 5))
            .Text(LOCTEXT("sample", "xyzSample"))
            ]
            ]
    ];
...
//add smaple xyzTiles
FReply FPLATEAUGmlSelectPanel::OnBtnSampleXyzTilesClicked() {

    //SampleSaveFig();

    VectorTileLoader loader;

    plateau::geometry::GeoCoordinate min(30, 120, 0) ;
    plateau::geometry::GeoCoordinate max(40, 140, 0);
    plateau::geometry::Extent extent(min, max);
    auto vectorTiles = loader.load(extent);

    //for (auto VectorTile : *vectorTiles)
    //{
    //    FString name = FString(VectorTile.image.c_str());
    //    UE_LOG(LogTemp, Warning, TEXT("Your string: %s"), *name);
    //}

    //auto coord = loader.getCoordinate_one();
    //UE_LOG(LogTemp, Warning, TEXT("xyz: %d, %d, %d"), coord.zoom_level, coord.column, coord.row);

    //auto coordinates = loader.getCoordinate_sam();
    //for (auto Coordinate : coordinates)
    //{
    //    UE_LOG(LogTemp, Warning, TEXT("xyz: %d, %d, %d"), Coordinate.zoom_level, Coordinate.column, Coordinate.row);
    //}

    //auto uris = loader.getUri(extent);
    //for (auto uri : *uris)
    //{
    //    FString URI = FString(uri);
    //    UE_LOG(LogTemp, Warning, TEXT("URI is : %s"), *URI);
    //}

    return FReply::Handled();
}

void FPLATEAUGmlSelectPanel::SampleSaveFig(TArray<uint8> fileData) {
    IImageWrapperModule& ImageWrapperModule = FModuleManager::LoadModuleChecked<IImageWrapperModule>(FName("ImageWrapper"));
    TSharedPtr<IImageWrapper> ImageWrapper = ImageWrapperModule.CreateImageWrapper(EImageFormat::JPEG);
    TArray<uint8> RawFileData;

    FString filePath = TEXT("D:/data/picture/IMG_3936.JPG");
    if (FFileHelper::LoadFileToArray(RawFileData, *filePath))
    {
            ...
            )
        {
            ...
            //UPackage::SavePackage(Package, Texture, RF_Public | RF_Standalone, *PackageFilename, GError, nullptr, true, true, SAVE_NoError);
        }
    }
}
