...
- 地形変換/高さ合わせ処理の成果物について、ランドスケープ化の成果物は名前の先頭に"DEM_"と付与された名前のランドスケープになります。高さ合わせした地物については、前と同じ名前のコンポーネントが生成されます。処理が完了したとき、結果は新しいコンポーネントとして配置されます。その地物のコンポーネント名は地物のGML IDとなります。ただし、主要地物内の同一マテリアル単位で分割結合した場合は該当コンポーネント名は"Material-(マテリアルID)"となります。
